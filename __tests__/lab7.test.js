describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    const updatedText = await button.getProperty('innerText');
    const updatedTextValue = await updatedText.jsonValue();
    expect(InitialTextValue).toBe('Add to Cart');
    expect(updatedTextValue).toBe('Remove from Cart');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
     ...
     */
     ...
     for (const productItem of productItems) {
         ...
         if (buttonText.includes("Remove from Cart")) {
             await button.click();
             await new Promise(resolve => setTimeout(resolve, 100));
         }
     }
